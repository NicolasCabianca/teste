...
function preload() {
  ...
  NPC6Img = loadImage("Img/NPC6.png");
  ...
  perdeuS = loadSound("perdeu.mp3")
  sfundo = loadSound("Sfundo2.mp3")
  Schave = loadSound("efeitoChave.mp3");
  spoder = loadSound("audioPDv.mp3");
...
function draw() {
  ...
  console.log("pintou o fundo de cinza")
  console.log("numero de jogadores: ", listaNivel2.legnth)
...
function pegarJogador() {
  database.ref("players").on("value", (data) => {
    ...
    for (var p in allPlayers) {
      ...
      console.log(allPlayers[p].nivel)
      if (allPlayers[p].nivel == 2) {
        sprites[allPlayers[p].personagem - 1].visible = true;
        listaNivel2.push(allPlayers[p]);
      }
    }
  });
}
